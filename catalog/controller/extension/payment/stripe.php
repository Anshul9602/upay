...
class ControllerExtensionPaymentStripe extends Controller {
	...
	private function getSubscriptionPlans($settings, $order_info) {
		...
		$decimal_factor = (in_array($settings['currencies_' . $currency], array('BHD','JOD','KWD','OMR','TND'))) ? 1000 : $decimal_factor;
		...
		foreach ($cart_products as $product) {
			...
			if ($this->config->get($prefix . 'tax_status')) {
				...
				$taxed_total = 0;
				foreach ($order_line_items as $line_item) {
					if (in_array($line_item['code'], array('avalara_integration', 'taxamo_integration', 'taxcloud_integration', 'taxjar_integration'))) {
	...
	public function createPaymentIntent() {
		...
		if (empty($this->session->data['order_id'])) {
			echo json_encode(array('error_message' => 'Error: missing order_id'));
			return;
		}
		...
		if (empty($order_info['email'])) {
			echo json_encode(array('error_message' => 'Error: please fill in your order information before attempting payment.'));
			return;
		}
		...
		if (!empty($customer_response['error']) || !empty($customer_response['deleted'])) {
			$this->db->query("DELETE FROM " . DB_PREFIX . $this->name . "_customer WHERE stripe_customer_id = '" . $this->db->escape($stripe_customer_id) . "' AND transaction_mode = '" . $this->db->escape($settings['transaction_mode']) . "'");
			$stripe_customer_id = '';
		}
		...
		if ($amount >= 0.5) {
			...
			if (!empty($payment_intent['error'])) {
				...
				echo json_encode(array('error_message' => 'Missing payment method'));
				return;
			} else {
				$json = array(
		...
		if ($store_card && $stripe_customer_id) {
			...
			if (!empty($make_default_response['error'])) {
				echo json_encode(array('error_message' => $make_default_response['error']['message']));
				return;
			}
		}
	...
	public function finalizePayment() {
		...
		if (strpos($payment_intent_id, 'order_status_id_') === 0) {
			...
			if (empty($settings['advanced_error_handling']) || $ignore_error_handling) {
				$this->addOrderHistory($order_id, str_replace('order_status_id_', '', $payment_intent_id));
			} else {
		...
		if (empty($json) && empty($settings['advanced_error_handling'])) {
	...
	public function webhook() {
		...
		if ($event['type'] == 'customer.deleted') {
			...
			// Find last order_id and original order_id
			$last_order_id = 0;
			...
			if (!empty($settings['subscription_fail_emails'])) {
				...
				$message .= '<br><b>Payment ID:</b> <a target="_blank" href="https://dashboard.stripe.com/' . ($event['livemode'] ? '' : 'test/') . 'payments/' . $webhook['payment_intent'] . '">' . $webhook['payment_intent'] . '</a>';
				...
				foreach (array('code', 'decline_code', 'message') as $parameter) {
					if (!empty($payment_intent_response['last_payment_error'][$parameter])) {
						$message .= '<br><b>' . ucwords(str_replace('_', ' ', $parameter)) . ':</b> ' . $payment_intent['last_payment_error'][$parameter];
					}
				}
			...
			$original_order_info = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order` WHERE order_id = " . (int)$original_order_id)->row;
			...
			if (version_compare(VERSION, '4.0', '>=')) {
				$default_address_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "address WHERE customer_id = " . (int)$opencart_customer['customer_id'] . " AND `default` = 1");
