	...
	public function deleteOrder($order_id) {
        ...
        public function getBA($order_id) {
            ...
            if ($order_query->num_rows) {
				...
				$analytics_bestprice_tax_class_id = $this->config->get("analytics_bestprice_tax_class_id");
				...
				foreach ($order_product_query->rows as $product) {
					...
					$analytics_bestprice_feed_id = $this->config->get("analytics_bestprice_feed_id");
					
					$product_data = $this->getProductData($product['product_id']);
					$product_id = $product_data[$analytics_bestprice_feed_id];
					
					if ($product['tax'] == 0) {
						$sum_product_price += $product['price'];
						$sum_product_tax += $this->calculateTax($analytics_bestprice_tax_class_id, $product['price']);
                    ...
                    if ($total['value'] > 0) {
                        if ($total['code'] == "shipping") {
							if (isset($this->session->data["shipping_method"]["tax_class_id"]) && $this->session->data["shipping_method"]["tax_class_id"] != 0) {
								...
								$order_shipping_tax = $this->calculateTax($analytics_bestprice_tax_class_id, $total['value']);
							}			
                        }
                    }
                }
