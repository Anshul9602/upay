...
class ModelToolExportImport extends Model {
	...
	protected function storeProductOptionValueIntoDatabase2( &$product_option_value ) {
		$product_id = $product_option_value['product_id'];
		$option_value_id = $product_option_value['option_value_id'];
		$quantity = $product_option_value['quantity'];
	
			$sql  = "UPDATE `".DB_PREFIX."product_option_value` SET quantity = $quantity";
			$sql .= " where product_id = $product_id AND option_value_id = $option_value_id";
			$this->db->query($sql);
	...
	protected function uploadProductOptionValues2( &$reader, $incremental, &$available_product_ids ) {
		// get worksheet, if not there return immediately
		$data = $reader->getSheetByName( 'ProductOptionValues' );
		if ($data==null) {
			return;
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$option_id = $this->getCell($data,$i,$j++,'');
			$option_id2 = $this->getCell($data,$i,$j++,'');
			...
			$this->storeProductOptionValueIntoDatabase2( $product_option_value);
			$previous_product_id = $product_id;
		}
	}
...
//		$col -= 1; // we use 1-based, PHPExcel uses 0-based column index, PhpSpreadsheet now uses 1-based column index
	...
	protected function validateProductOptionValues2( &$reader ) {
		$data = $reader->getSheetByName( 'ProductOptionValues' );
		if ($data==null) {
			return true;
		}
		$expected_heading = array( "product_id", "model", "option", "option_value_id", "option_value", "quantity");
		$expected_multilingual = array();
		return $this->validateHeading( $data, $expected_heading, $expected_multilingual );
	}
	...
	protected function validateUpload2( &$reader )
	{
		...
		// worksheets must have correct heading rows
		if (!$this->validateProductOptionValues2( $reader )) {
			$this->log->write( $this->language->get('error_product_option_values_header') );
			$ok = false;
		}
	...
	public function upload_inventory( $filename, $incremental=true ) {
		// we use our own error handler
		global $registry;
		$registry = $this->registry;
		set_error_handler('error_handler_for_export_import',E_ALL);
		...
		try {
			...
			$available_category_ids = array();
			$available_customer_ids = array();
			$this->uploadProductOptionValues2( $reader, $incremental, $available_product_ids );
			print_r('ary');
			return true;
		} catch (Exception $e) {
			$errstr = $e->getMessage();
	...
	protected function getProductOptionValues2( $min_id, $max_id ) {
		$language_id = $this->getDefaultLanguageId();
		$sql  = "SELECT ";
		$sql .= "  p.product_id, p.model,  pd.name, pov.option_id, pov.option_value_id, pov.quantity, pov.subtract, od.name AS `option`, ovd.name AS option_value, ";
		$sql .= "  pov.price, pov.price_prefix, pov.points, pov.points_prefix, pov.weight, pov.weight_prefix ";
		$sql .= "FROM ";
		$sql .= "( SELECT p1.product_id, p1.model";
		$sql .= "  FROM `".DB_PREFIX."product` p1 ";
		if ($this->posted_categories) {
			$sql .= "LEFT JOIN `".DB_PREFIX."product_to_category` pc ON pc.product_id=p1.product_id ";
		}
		if (isset($min_id) && isset($max_id)) {
			$sql .= " AND WHERE p1.product_id BETWEEN $min_id AND $max_id ";
			if ($this->posted_categories) {
				$sql .= "AND pc.category_id IN ".$this->posted_categories." ";
			}
		} else if ($this->posted_categories) {
			$sql .= " AND WHERE pc.category_id IN ".$this->posted_categories." ";
		}
		if ($this->posted_manufacturers) {
			$sql .= (strpos($sql," WHERE ",0)===false) ? "WHERE " : "AND ";
			$sql .= "p1.manufacturer_id IN ".$this->posted_manufacturers." ";
	...
	protected function populateProductOptionValuesWorksheet2( &$worksheet, &$price_format, &$box_format, &$weight_format, &$text_format, $min_id=null, $max_id=null ) {
		// Set the column widths
		$j = 1;
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('product_id')+1);
		if ($this->config->get( 'export_import_settings_use_option_id' )) {
		...
		$i += 1;
		$j = 1;
		$product_option_values = $this->getProductOptionValues2( $min_id, $max_id );
		foreach ($product_option_values as $row) {
			//if($row['option']!="Metal Color"){
				$worksheet->getRowDimension($i)->setRowHeight(13);
				$data = array();
				$data[$j++] = $row['product_id'];
				$data[$j++] = $row['model'];
		 ...
		 $rows=null;
		   ...
		   $max_id=null;
			...
			try {
				...
				$this->populateProductOptionValuesWorksheet2( $worksheet, $price_format, $box_format, $weight_format, $text_format, $min_id, $max_id );
				$worksheet->freezePaneByColumnAndRow( 2, 2 );

				
				// redirect output to client browser
				$datetime = date('Y-m-d');
				$filename = 'inventory-'.$datetime.'.xlsx';
