...
class ControllerExtensionExportImport extends Controller { 
	...
	public function upload_inventory() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' ) {
			if (isset($this->request->files['upload_inventory'] )) {
				$file = $this->request->files['upload_inventory']['tmp_name'];
				if ($this->model_extension_export_import->upload_inventory($file)) {
					$this->session->data['success'] = $this->language->get('text_success');
					$this->response->redirect($this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'], $this->ssl));
				}
		//		$this->model_extension_export_import->upload_inventory($file);
			}
		}
	}
	...
	public function download_inventory() {
		...
		$this->document->setTitle($this->language->get('heading_title'));
		$this->load->model( 'extension/export_import' );
		$this->model_extension_export_import->download_inventory('p', null, null, null, null);
			
		
		$this->response->redirect($this->url->link( 'catalog/product', 'user_token='.$this->request->get['user_token'], $this->ssl) );
	}
